...
cc.Class({
  ...
  resumeHealth (_num) {
    this.node.runAction(cc.sequence(cc.callFunc(() => {
      GameApp.uiManager.showGameObject('InfoLabel', (node) => {
        ...
        const seq = cc.sequence(init, cc.spawn(cc.fadeIn(0.2), cc.scaleTo(0, 1.5), cc.moveTo(0.3, cc.v2(0, 190))), cc.fadeOut(0.3), cc.callFunc(() => {
          node.destroy()
        }))
        node.runAction(seq)
      }, this.node)
  ...
  initSkinShow () {
    ...
    const arr = GameApp.dataManager.userData.choosedSkinId < 21 ? ['await'] : ['await_fight_1']
  ...
  move (dt, _flash) {
    if (_flash) {
      const _borderGroup = GameApp.uiManager.getGame('GameMap').getComponent('GameMap').enemySpawnPosGroupNode.children
      // console.log(_borderGroup)
      const newPos1 = this.node.position.add(this.moveDir.mul(1000))
  ...
  setSpeedType (_type) {
    ...
    if (this._speedType != _type) {
      ...
      const arr = GameApp.dataManager.userData.choosedSkinId < 21 ? ['await', 'run2', 'run2'] : ['await_fight_1', 'run', 'run']
  ...
  shoot () {
    ...
    if (GameApp.dataManager.reduceAmo()) {
      ...
      let power = Math.ceil(this.gunData.power * (GameApp.dataManager.globalData.curDamage + GameApp.dataManager.getEquipItemAttr(EquipType.damage)))
      let isCrit = false
  ...
  pickUpWeapon () {
    const _kind = this.tempGroundWeapon.itemType._kind
    if (!this._haveGun) {
  ...
  beDamage (_power, _belongIndex, _belongName, _isCrit) {
    GameApp.uiManager.showGameObject('InfoLabel', (node) => {
      ...
      // 皮肤减伤+装备减伤
      const _isDef = Math.ceil(_power * GameApp.dataManager.globalData.curDef) + Math.ceil(_power * GameApp.dataManager.getEquipItemAttr(EquipType.def))
      _power -= _isDef
    ...
    if (GameApp.dataManager.reduceHp(_power)) {
      ...
      const theRank = GameApp.dataManager.globalData.allRoleArr.length
