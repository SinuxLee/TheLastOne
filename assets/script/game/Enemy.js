...
cc.Class({
  ...
  enemyEquipBoxItem () {
    ...
    const _rank = this.getItemAttrArr[_selectIndex].rank + 1
  ...
  updateStarShow () {
    ...
    if (_sum == 0) {
      ...
      const level = parseInt((_sum - 1) / 3)
      const starNum = _sum - level * 3
  ...
  move (dt) {
    ...
    const theAngle = 90 - cc.misc.radiansToDegrees(Math.atan2(this.moveDir.y, this.moveDir.x))
    if (theAngle > 180 || theAngle < 0) {
      this.roleAnim.node.scaleX = -1
    } else {
      this.roleAnim.node.scaleX = 1
    }
    const newPos = this.node.position.add(this.moveDir.mul(this._moveSpeed * dt))
  ...
  doParser (dt) {
    ...
    if (GameApp.dataManager.globalData.gasConfig != null) {
      const dir = cc.v2(GameApp.dataManager.globalData.gasConfig.safePosition).sub(cc.v2(this.node.position))
    ...
    if (this._desTime < 0) {
      ...
      if (!this._isGas && !this._mapNoGun && !this._haveGun) {
        ...
        if (!this._isGas && GameApp.dataManager.globalData.isInGame && !this._mapNoBox && this._haveGun) {
          ...
          // console.log("巡逻")
          this._desTime = Tools.randomNum(1, 3)
          const desDir = cc.v2(0, 1).rotate(cc.misc.radiansToDegrees(Tools.randomNum(0, 360)))
  ...
  searchGun () {
    ...
    for (let i = 0; i < allGunArr.length; i++) {
      const distance = cc.v2(allGunArr[i].position).sub(cc.v2(this.node.position)).mag()
      if (distance < minDis) {
        if (GameApp.dataManager.globalData.gasConfig != null) {
          const dirDistance = cc.v2(GameApp.dataManager.globalData.gasConfig.safePosition).sub(cc.v2(allGunArr[i].position)).mag()
  ...
  searchBox () {
    ...
    for (let i = 0; i < allBoxArr.length; i++) {
      const distance = cc.v2(allBoxArr[i].position).sub(cc.v2(this.node.position)).mag()
      if (distance < 10) {
      ...
      if (distance < minDis) {
        if (GameApp.dataManager.globalData.gasConfig != null) {
          const dirDistance = cc.v2(GameApp.dataManager.globalData.gasConfig.safePosition).sub(cc.v2(allBoxArr[i].position)).mag()
          if (dirDistance > GameApp.dataManager.globalData.gasConfig.gasCircle / 2) {
            continue
          }
        }
  ...
  setSpeedType (_type) {
    ...
    if (this._speedType != _type) {
      ...
      const arr = this._choosedSkinId < 21 ? ['await', 'run2', 'run2'] : ['await_fight_1', 'run', 'run']
  ...
  aimToNearest () {
    ...
    let index = -1
    for (let i = 0; i < allRoleArr.length; i++) {
      const distance = cc.v2(allRoleArr[i].node.position).sub(cc.v2(this.node.position)).mag()
    ...
    if (index == -1) {
      ...
      const aimEnemy = allRoleArr[index].node
  ...
  shoot () {
    ...
    if (this.reduceAmo()) {
      ...
      let power = Math.ceil(this.gunData.power * (this._curDamage + this.equipItemAttr[EquipType.damage]))
      let isCrit = false
      ...
      if (this.gunData.weaponid == 1002) {
        ...
        const bullet1 = cc.instantiate(this.bulletPrefab)
        const bullet2 = cc.instantiate(this.bulletPrefab)
        const bullet3 = cc.instantiate(this.bulletPrefab)
        ...
        const bulletC2 = bullet2.getComponent('Bullet')
        ...
        const bulletC3 = bullet3.getComponent('Bullet')
  ...
  onBeginContact (contact, self, other) {
    ...
    if (other.tag == Tags.item) {
      const groundItem = other.node.getComponent('GroundItem')
      if (groundItem.itemType._type == ItemType.weapon) {
        const _kind = groundItem.itemType._kind
        if (!this._haveGun) {
      ...
      const bC = other.node.getComponent('Bullet')
  ...
  beDamage (_power, _belongIndex, _belongName, _isCrit, _weaponid) {
    GameApp.uiManager.isInMapSight(this.node) && GameApp.uiManager.showGameObject('InfoLabel', (node) => {
      ...
      const _isDef = Math.ceil(_power * this._curDef) + Math.ceil(_power * this.equipItemAttr[EquipType.def])
      _power -= _isDef
      let str = '<color=red>-' + _power + '</color>'
      if (_isCrit) {
      ...
      const desPos = cc.v2(Tools.randomNum(60, 100), Tools.randomNum(60, 100))
      const bezier = [originPos, cc.v2(originPos.x + desPos.x - 20, originPos.y + desPos.y + 20), originPos.add(desPos)]
      const seq = cc.sequence(cc.spawn(cc.fadeIn(0.2), cc.scaleTo(0.3, 1.5), cc.bezierTo(0.3, bezier)), cc.delayTime(0.5), cc.fadeOut(0.3), cc.callFunc(() => {
        node.destroy()
      }))
      node.runAction(seq)
    ...
    if (this.reduceHp(_power)) {
      ...
      if (_belongIndex == -1) {
        ...
        const killNum = GameApp.dataManager.addKillNum(_belongIndex, _belongName)
        if (_belongName == GameApp.dataManager.userData.playerName) {
          _belongName = '<color=#0fffff>' + _belongName + '</color>'
        }
        if (killNum > 1) {
          const soundIndex = killNum > 5 ? 5 : killNum
      ...
      if (this._haveGun) {
        const theParentC = GameApp.uiManager.gameRoot.children[0].getComponent('GameMap')
        GameApp.uiManager.showGameObject('GroundItem', (node) => {
          const ddd = theParentC.allGunNode.convertToNodeSpaceAR(this.node.convertToWorldSpaceAR(cc.v2(0, 0)))
      ...
      //     }
      // }
      const arr = this._choosedSkinId < 21 ? ['dead'] : ['dead2']
