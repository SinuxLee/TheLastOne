...
cc.Class({
    ...
    if (this._gasState == 2) {
      ...
      if (!Tools.isIntersect(this.gasPanel.children[1].position, this.gasPanel.children[1].width / 2, this.safePosition, this.safeCircle / 2)) {
        ...
        if (Math.abs(this.gasPanel.children[1].width - this.safeCircle) > 0.01) // 外圈和内圈圆心重合,半径相同
        {
          ...
          // x^2+y^2 = 1
          // x^2 = 1/(k^2+1)
          const k = (this.gasPanel.children[1].y - this.safePosition.y) / (this.gasPanel.children[1].x - this.safePosition.x)

          const x_off = delt * parseFloat(Math.sqrt(1 / (k * k + 1)))
  ...
  dropItBoom (boomArr, _index) {
    ...
    const _delay = Tools.randomNum(0, 2, true)
    this.node.runAction(cc.sequence(cc.delayTime(_delay), cc.callFunc(() => {
      GameApp.uiManager.showGameObject('Boom', (node) => {
        ...
        node.children[1].runAction(cc.sequence(cc.delayTime(1), cc.callFunc(() => {
          ...
          const coli = node.getComponent(cc.PhysicsBoxCollider)
          coli.enabled = true
          const theAnim = node.children[1].getComponent(sp.Skeleton)
  ...
  initMapShow () {
    ...
    const wallLayer = this.tiledMap.getLayer('wall')
    this.tiledMap.getObjectGroup('dynamic')._objects.forEach(element => {
      const obj = cc.instantiate(this.objectPrefab)
      obj.getComponent(cc.Sprite).spriteFrame = this.mapAtlas.getSpriteFrame(element.name.split('.')[0])
      obj.parent = this.allColliderNode
      const desPos = wallLayer.getPositionAt(this.getTilePos(cc.v2(element.x, element.y)))
      obj.setPosition(desPos)
      const ddd = this.allObjectNode.convertToNodeSpaceAR(obj.convertToWorldSpaceAR(cc.v2(0, 0)))
      obj.parent = this.allObjectNode
      obj.setPosition(ddd)
    })
    this.tiledMap.getObjectGroup('collision')._objects.forEach(element => {
      const obj = cc.instantiate(this.colliderPrefab)
      const coli = obj.getComponent(cc.PhysicsBoxCollider)
  ...
  showParatrooperPlayer (_chooseType, _bili) {
    GameApp.uiManager.showGameObject('ParatrooperPlayer', (node) => {
      const pC = node.getComponent('ParatrooperPlayer')
  ...
  showParatrooperEnemy (_chooseType, _bili) {
    GameApp.uiManager.showGameObject('ParatrooperEnemy', (node) => {
      ...
      node.setPosition(theX, theY)
      const desDir = cc.v2(0, 10).rotate(cc.misc.radiansToDegrees(Tools.randomNum(0, 360)))
  ...
  showCloud (_speedUpJump) {
    ...
    const each0_05 = 0.3 / (3.6 + _jumpTime * 3) / 100 * 5
    const act = cc.sequence(cc.callFunc(() => {
  ...
  gameBegin (_speedUpJump) {
    ...
    const landUpPos = this.paratrooperNode.children[0].position
  ...
  showGun () {
    ...
    this.tiledMap.getObjectGroup('grass')._objects.forEach(element => {
      ...
      const obj = cc.instantiate(this.groundItemPrefab)
      obj.parent = this.allColliderNode
      const desPos = wallLayer.getPositionAt(this.getTilePos(cc.v2(element.x, element.y)))
      obj.setPosition(desPos)
      const ddd = this.allGunNode.convertToNodeSpaceAR(obj.convertToWorldSpaceAR(cc.v2(0, 0)))
    ...
    this.otherGunPos.children.forEach(posNode => {
      const obj = cc.instantiate(this.groundItemPrefab)
      const ddd = this.allGunNode.convertToNodeSpaceAR(posNode.convertToWorldSpaceAR(cc.v2(0, 0)))
      obj.parent = this.allGunNode
      obj.setPosition(ddd)
      const _param = {
        _type: ItemType.weapon,
  ...
  getTilePos (posInPixel) {
    const mapSize = this.tiledMap.node.getContentSize()
  ...
  showPlayer (landUpPos) {
    GameApp.uiManager.showGameObject('Player', (node) => {
      ...
      const c = node.getComponent('Player')
  ...
  showEnemy () {
    const tempPosArr = []
    const standardArr = this.enemySpawnPosGroupNode.children
    // var xRangeArr = []
    ...
    for (var i = 0; i < tempPosArr.length; i++) {
      ...
      GameApp.uiManager.showGameObject('Enemy', (node) => {
        ...
        const c = node.getComponent('Enemy')
  ...
  sortAll () {
    const cloneObjArr = this.allObjectNode.children.concat()
    const tempDict = []
    const tempIndex = []
    for (const i in cloneObjArr) {
