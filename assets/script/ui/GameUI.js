...
cc.Class({
  ...
  protectCcountDownAnim () {
    ...
    const seq = cc.sequence(cc.scaleTo(0.2, 1.5), cc.scaleTo(0.3, 1), cc.delayTime(0.5), cc.callFunc(() => {
  ...
  quietGasState () {
    if (this._gasCountDownTimer < 16) {
      ...
      let theWidth = this.gasNode.width
      if (theWidth == 300) {
        theWidth = 204
      }
  ...
  reduceCircleAndTip (dt) {
    ...
    if (!Tools.isIntersect(this.gasNode.position, this.gasNode.width / 2, this.safeNode.position, this.safeCircle / 2)) {
      ...
      if (Math.abs(this.gasNode.width - this.safeCircle) > 0.05) // 外圈和内圈圆心重合,半径相同
      {
        ...
        // x^2+y^2 = 1
        // x^2 = 1/(k^2+1)
        const k = (this.gasNode.y - this.safeNode.y) / (this.gasNode.x - this.safeNode.x)

        const x_off = dt * 10 * parseFloat(Math.sqrt(1 / (k * k + 1)))
  ...
  dropBox () {
    ...
    const theNumArr = [10, 6, 3, 2]
    for (let i = 0; i < theNumArr[this._dropTime]; i++) {
      GameApp.uiManager.showGameObject('MipBox', (node) => {
        this._mipBoxGroup.push(node)
        const thePos = Tools.pointOfRandom(this.safeNode.position, this.safeCircle / 2, 0)
        node.setPosition(thePos)
        const paramPos = this.mipNode.convertToNodeSpaceAR(node.convertToWorldSpaceAR(cc.v2(0.5, 0.5)))
  ...
  showBoxItemUI () {
    ...
    const arr = [0, 1, 2, 3]
    const randNum = Tools.randomNum(3, 4)
    const randArr = Tools.getRandomAmountElementUnRepeat(arr, randNum).nodeArr
    // console.log(randArr)
    randArr.forEach(element => {
      const _rank = GameApp.dataManager.globalData.getItemAttrArr[element].rank
      if (_rank == 3) {
  ...
  boxItemUIBtnClick (eventTouch, customEventData) {
    const _selectIndex = parseInt(customEventData) - 1
  ...
  updateEquipShowUI () {
    const arr = GameApp.dataManager.getEquipShowAttr()
    for (const i in this.equipAttrNodeGroup) {
      if (arr[i] == null) {
  ...
  updateAllRoleNumUIShow () {
    ...
    if (theRank == 1) {
      const theRole = GameApp.dataManager.globalData.allRoleArr[0]
  ...
  updateRankShow () {
    const theArr = GameApp.dataManager.globalData.inGameKillNum.concat()
    theArr.sort((a, b) => b._killNum - a._killNum)
    for (let i = 0; i < 5; i++) {
      let theName = theArr[i]._belongName
  ...
  soundBtnClick () {
    GameApp.audioManager.playEffect('click')
    const onoff = !GameApp.audioManager._effectOn
