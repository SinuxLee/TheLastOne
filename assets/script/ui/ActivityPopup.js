...
cc.Class({
  ...
  onLoad () {
    const a = GameApp.dataManager.globalData.onLineGiftCurNum
  ...
  updateOnlineTask () {
    let finishAll = true
    for (const i in this.page1.children) {
  ...
  updateOnlineTaskBtnShow () {
    const _minuteTime = GameApp.dataManager.getOnlineTimeMinute()
    for (const i in this.page1.children) {
  ...
  updateActivityTask () {
    for (const i in this.activityTaskNum) {
  ...
  updateActivityTaskShow () {
    const arr1 = ['累计使用手枪击杀' + this.activityTaskNum[0] + '人',
  ...
  onLineTaskBtnClick (eventTouch, customEventData) {
    const selectId = parseInt(customEventData)
