...
cc.Class({
  ...
  init (_isWin, rank) {
    ...
    const killNum = GameApp.dataManager.globalData.inGameKillNum[0]._killNum

    GameApp.dataManager.userData.allKillNum += killNum

    this.killUI.string = killNum
    const rankCoin = (30 - rank) * 50
    const killCoin = 100 * killNum
    const allCoin = rankCoin + killCoin
    ...
    if (window.tt) {
      Utils.addVideo('addVideo', function () { // 看完了
        ...
        GameApp.uiManager.showPopup('OpenBoxPopup', (node) => {
          const getNum = Tools.randomNum(100, 1000)
