...
window.Tools = {
  ...
  isCrit (_num) {
    ...
    const result = this.getRandomAmountElementUnRepeat(arr, _num)
    if (result == null) {
      return false
    } else {
  ...
  pointOfRandom (point, radius_outer, radius_inner) {
    let x = Tools.randomNum(Math.floor(point.x - (radius_outer - radius_inner)), Math.floor(point.x + (radius_outer - radius_inner)))
    let y = Tools.randomNum(Math.floor(point.y - (radius_outer - radius_inner)), Math.floor(point.y + (radius_outer - radius_inner)))
  ...
  isIntersect (p_outer, r_outer, p_inner, r_inner) {
    ...
    const distance = parseFloat(Math.sqrt((p_outer.x - p_inner.x) * (p_outer.x - p_inner.x) + (p_outer.y - p_inner.y) * (p_outer.y - p_inner.y)))
  ...
  jumpOneByOne: function (target) {
    const nodeNum = target.childrenCount
  ...
  calculation: function (num, rate, strName) {
    const n1 = Math.floor(num / rate)
    const n2 = Math.floor(num % rate / rate * 10)
    if (n2 === 0) {
      return n1 + strName
    } else {
