...
cc.Class({
  ...
  startLoading: function () {
    ...
    this.loadAll(function (completedCount, totalCount) {
      const per = completedCount / totalCount
      if (per && !isNaN(per)) {
      ...
      // 读取用户信息;
      const userData = cc.sys.localStorage.getItem('EatChicken_UserData')
  ...
  loadAll (cbProgress, cbComplete) {
    ...
    let ii = 0
    // 加载预制体配置路径文件;
    for (const name in PREFAB_PATH) {
      paths.push(PREFAB_PATH[name])
      ii++
    }
    const prefabCount = ii
    let jj = 0
    // 加载JSON配置文件;
    for (const name2 in JSON_PATH) {
  ...
  showGameObject (name, cb, parentNode) {
    ...
    const node = cc.instantiate(this._Prefabs[name])
  ...
  popUI () {
    const uis = this.uiRoot.children
    if (uis.length > 0) {
  ...
  isInMapSight (_target) {
    const cameraWidth = this.mapCamera.node.width / 0.9
    const cameraHeight = this.mapCamera.node.height / 0.9
    const thePos = this.mapCamera.getWorldToScreenPoint(_target.convertToWorldSpaceAR(cc.v2(0, 0)))
  ...
  popPopup () {
    const popups = this.popupRoot.children
    if (popups.length > 0) {
  ...
  closePopup (name) {
    for (const node of this.popupRoot.children) {
      if (node.name === name) {
  ...
  showToast (s, callback, num) {
    ...
    if (this.toastRoot.childrenCount > 0) {
      for (let i = this.toastRoot.childrenCount - 1; i >= 0; i--) {
        const moveUp = cc.moveBy(0.1, cc.v2(0, toast.height))
