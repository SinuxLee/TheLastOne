...
cc.Class({
  ...
  initSomeAttr () {
    ...
    const getState = cc.sys.localStorage.getItem('EatChicken_dailyGotState')
    ...
    let tempOnLineGifts = cc.sys.localStorage.getItem('EatChicken_onLineGifts')
    ...
    let tempOnLineGiftCurNum = cc.sys.localStorage.getItem('EatChicken_onLineGiftCurNum')
    ...
    let tempactiveValue = cc.sys.localStorage.getItem('EatChicken_activeValue')
    ...
    let tempactivityGifts = cc.sys.localStorage.getItem('EatChicken_activityGifts')
    ...
    let tempactivityNum = cc.sys.localStorage.getItem('EatChicken_activityNum')
    ...
    let tempactivityCurNum = cc.sys.localStorage.getItem('EatChicken_activityCurNum')
    ...
    let tempactivityCurCoin = cc.sys.localStorage.getItem('EatChicken_activityCurCoin')
    ...
    let tempprogressGifts = cc.sys.localStorage.getItem('EatChicken_progressGifts')
  ...
  reduceCoin (_num) {
    const a = this.userData.coinNum - _num
    if (a < 0) {
      return false
    } else {
  ...
  addSkinAttr () {
    const curSkinData = this.getSkinDataById(this.userData.choosedSkinId)
  ...
  reduceHp (_num) {
    const a = this.globalData.curHp - _num
    if (a <= 0) {
  ...
  reduceAmo () {
    const a = this.globalData.curAmoNum - 1
    if (a < 0) {
  ...
  equipBoxItem (_index) {
    const _rank = this.globalData.getItemAttrArr[_index].rank + 1
  ...
  getEquipShowAttr () {
    const arr = []
    for (const i in this.globalData.getItemAttrArr) {
      const _rank = this.globalData.getItemAttrArr[i].rank
      if (_rank == 0) {
        arr.push(null)
      } else {
  ...
  getOnlineTimeMinute () {
    let minuteTime = 0
    const newtime = Tools.toTimeString2(this.globalData.onLineTime)
    minuteTime = Math.floor(newtime.hour * 60 + newtime.minute)
    return minuteTime
  },
  addActiveValue (_num) {
    let a = this.globalData.activeValue + _num
    if (a > 120) {
      a = 120
    }
    this.globalData.activeValue = a
    const arr = [30, 60, 90, 120]
    for (const i in arr) {
