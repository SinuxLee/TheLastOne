...
util._getShareObject = function (from, _callSuccess, _callFailed) {
  const title = '史上最难的游戏，玩过的人都说春运抢票不算难！'
  let query = null
  let desc = null
  const obj = Tools.getRandomElement(shareTxts)
  if (obj) {
    desc = obj.txt
...
util.addBanner = function (_id, _sy, _WinHeight, _call) {
  ...
  const phone = tt.getSystemInfoSync()
  const w = phone.screenWidth
  const h = phone.screenHeight
  if (util.m_banner) {
